    ...
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-[#EDE8E0] shadow-xs">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          <Link href="/book">
            <Button size="sm" className="ml-3 bg-primary text-white hover:bg-primary/90 rounded-lg">Book Now</Button>
          </Link>
        </div>

        <button
          className="md:hidden p-2 rounded-md hover:bg-[#F0EBE1] transition-colors"
  ...
  return (
    <Link
      ...
      className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
        active
          ? "bg-[#F0EBE1] text-primary"
          : "text-[#5C5043] hover:text-[#1A1209] hover:bg-[#F5F1EB]"
      }`}
    >
      {label}
    </Link>
...
function MobileNavLink({ href, label, onClick }: { href: string; label: string; onClick: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block px-3 py-2.5 rounded-md text-sm font-medium text-[#1A1209] hover:bg-[#F5F1EB] transition-colors"
    >
      {label}
    </Link>
  );
